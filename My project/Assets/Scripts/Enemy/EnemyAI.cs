...
public class EnemyAI : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        Vector3 between = Vector3.Scale(target.position - transform.position, new Vector3(1, 0, 1));
        

        if ((target.position - transform.position).magnitude > 5)
        {
            moveDirection = between;
        }
        else if ((target.position - transform.position).magnitude < 4)
        {
            moveDirection = -(between);
        } 
        else
